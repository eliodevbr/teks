...
inputs = {
  ...
  cluster_addons = {
    ...
    ebs-csi = {
      addon_version     = "v1.7.0-eksbuild.0"
      resolve_conflicts = "OVERWRITE"
    }
  }
  ...
  eks_managed_node_groups = {
    ...
    "${include.root.locals.merged.application}-a" = {
      desired_size            = 1
      ami_type                = "AL2_x86_64"
      platform                = "linux"
      instance_types          = ["${include.root.locals.merged.instance_type}"]
      ...
      pre_bootstrap_user_data = <<-EOT
        ...
        export KUBELET_EXTRA_ARGS="--max-pods=${run_cmd("/bin/sh", "-c", "../../../../../../../tools/max-pods-calculator.sh --instance-type ${include.root.locals.merged.instance_type} --cni-version 1.10.1 --cni-prefix-delegation-enabled")}"
        EOF
    ...
    "${include.root.locals.merged.application}-b" = {
      ...
      bootstrap_extra_args       = <<-EOT
        "max-pods" = ${run_cmd("/bin/sh", "-c", "../../../../../../../tools/max-pods-calculator.sh --instance-type ${include.root.locals.merged.instance_type} --cni-version ${include.root.locals.merged.cni_version} --cni-prefix-delegation-enabled")}
        EOT
      block_device_mappings = {
        root = {
          device_name = "/dev/xvda"
    ...
    "${include.root.locals.merged.application}-c" = {
      ami_type                   = "BOTTLEROCKET_x86_64"
      platform                   = "bottlerocket"
      instance_types             = ["${include.root.locals.merged.instance_type}"]
